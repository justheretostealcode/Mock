...
import competition_model_erich as competition
...
class execution_wrapper:
    def __init__(self, command_dict):
        ...
        self.pre = dict(
            {k: (lambda *args, **kwargs: (args, kwargs)) for k, v in self.main.items()}
        )
    ...
    def register_pre(self, key, key_or_fun, compose=True):
        if isinstance(key_or_fun, str) and key_or_fun in self.main.keys():
            ...
            self.pre[key] = key_or_fun
        else:
            raise ValueError(
                "Supplied argument is neither callable nor a registered function."
            )
...
# input encoding dict
input_encoding = dict({0: "a", 1: "b", 2: "c"})

# reverse input encoding dict
reverse_input_encoding = dict({"a": 0, "b": 1, "c": 2})

# bounding submodes
bounding_modes = dict({"heuristic": 1, "ita": 2})
...
class nor_circuit:
    def __init__(self, structure, assignment, library, solver=None):
        ...
        self.dev_idx = dict(
            zip(
                [e["name"] for e in self.lib.c["devices"]],
                list(range(len(self.lib.c["devices"]))),
            )
        )
        self.tf_idx = dict(
            {e["name"]: tuple() for e in self.lib.c["transcription_factors"]}
        )
        ...
        for gate in self.lib.c["gates"]:
            ...
            self.g_idx[gate["name"]] = tuple(
                int(self.dev_idx[dev_name]) for dev_name in gate["associated_devices"]
            )
        for tf in self.lib.c["transcription_factors"]:
            self.tf_idx[tf["name"]] = tuple(
                int(self.dev_idx[dev_name]) for dev_name in tf["associated_devices"]
            )
        ...
        for k, v in self.assignment.map_gtod.items():
            ...
            if v.startswith("output"):  # implicit or / reporter TF
                self.gates[self.node_idx[k]] = _perturbed_implicit_or_gate(
                    k, v, self.parameters[self.dev_idx[v]]
                )
            elif v.startswith("input"):  # an input
                self.gates[self.node_idx[k]] = _base_gate(k, v)
            else:  # assigned NOR gate
                self.gates[self.node_idx[k]] = _perturbed_nor_gate(
                    k, v, self.parameters[self.dev_idx[v]]
                )
    ...
    def propagate(self, a):
        ...
        for n in range(len(self.gates)):
            ...
            # prepare second input, i.e. Q_p for this gate.
            q_selection = np.ones_like(a[:, 1])
            q_selection[n] = 0.0
            input[1] = np.dot(q_selection, a[:, 1])
            # prepare third input, i.e. S for this gate.
            input[2] = a[n, 2]
    ...
    def set_truthtable(self, ttix=0):
        ...
        if self.structure.gate_truthtables is None:
            ...
            return
        for n in range(len(self.gates)):
            self.bound_env[n] = self.structure.gate_truthtables[self.gates[n].node][
                ttix
            ]
    ...
    def solve(self, tol, max_iter):
        if self.solver is not None:
            ...
            return (
                values[self.node_idx[list(self.structure.outputs)[0]], 1],
                err,
                iter,
            )
        return None
...
class circuit_structure:
    def __init__(self, str):
        ...
        if circ is not None:
            ...
            self.adjacency = {
                "in": dict_from_list(self.nodes, set()),
                "out": dict_from_list(self.nodes, set()),
            }
    ...
    def combinational_order(self):
        if self.order is None:
            ...
            visited = dict(
                zip(list(self.nodes), [False for _ in range(len(self.nodes))])
            )
...
class _perturbed_implicit_or_gate(_base_gate):
    # represents a NOR gate in the circuit
    def __init__(self, node, dev, parameters):
        self.node = node
        self.dev = dev
        self.type = 1
        self.Q_y = parameters["Q_y"]
        self.S = parameters["S"]
        self.y0 = 0

    def initial_out(self, input):
        # initial output is the Hill function
        output = np.zeros(3)
        output[0] = input[0]
        # set outputs to appropriate values
        self.y0 = output[0]
        output[1] = 0.0
        output[2] = 0.0
        return output
    ...
    def out(self, input):
        ...
        # input[0] = input promoter activity
        # input[1] = Q_p of this gate
        # input[2] = S of this gate
        # output contains two values
        # output[0] = perturbed output promoter activity
        # output[1] = new Q_m of this gate
        # output[2] = new S of this gate
        output = np.zeros(3)
        # TODO: Erich
        output[0] = input[0]  # y_out = y_in
        lst = competition.perturbed_gate_output_of_output(
            output[0], input[1], input[2], self.Q_y, self.y0
        )
        output[0] = lst[0]
        output[1] = lst[1]
        output[2] = lst[2]
...
class _perturbed_nor_gate(_base_gate):
    # represents a NOR gate in the circuit
    def __init__(self, node, dev, parameters):
        self.node = node
        self.dev = dev
        self.type = 0
        # TODO was mit input gates machen?
    ...
    def initial_out(self, input):
        # initial output is the Hill function
        output = np.zeros(3)
        output[0] = self.y_min + (self.y_max - self.y_min) / (
            1 + (input[0] / self.K) ** self.n
        )
        # set outputs to appropriate values
        self.y0 = output[0]
        output[1] = 0.0
    ...
    def out(self, input):
        ...
        lst = competition.perturbed_gate_output_of_gate(
            output[0],
            input[1],
            input[2],
            self.y_max,
    ...
    def set_initial_value(self, initial_value):
        ...
        for node in order:
            ...
            if not self.circuit.mutable[gix]:
                vals[gix, 0] = self.values[gix, 0]
                continue
            gate = self.circuit.gates[gix]
            # BEGIN: this block shall later become the wire function
            input = np.empty(self.circuit.wire_capacity)
            input[0] = np.dot(self.circuit.w[:, gix], vals[:, 0])
            # prepare second input, i.e. Q_p for this gate to zero for initial run.
            input[1] = 0.0
            # prepare third input, i.e. S for this gate to zero for initial run.
            input[2] = 0
            # END: this block shall later become the wire function
            output = gate.initial_out(input)
            vals[gix, 0] = output[0]
        self.values = deepcopy(vals)
