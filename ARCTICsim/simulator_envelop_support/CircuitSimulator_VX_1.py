...
def startSimulation_VEC(assignment, simData, simSpec):
    ...
    def startSimulation_VEC(nodeOrder, circuit, assignment, truthTable, responseFunctions, particles, circuitSim):
        ...
        simulatorFunction = simulateCircuit_VEC
        if (simContext["substitute"]):
            simulatorFunction = simulateSubstitutedCircuit_VEC

        # Iterates over every particle for each parameter, while the circuit is simulated for each such particle
        #VEC: no need anymore for: for iX in range(numberOfParticlesToUse):
            ...
            for node in usedRealisations:
                ...
                try:  # Proof, whether the try catch can be replaced with if("particle_parameters" in respFunct):
                    ...
                    for parameter in respFunc["particle_parameters"]:
                        ...
                        responseFunctions[node]["parameters"][parameter] = particles[node][parameter]
                except KeyError:
                    zero = 0
    ...
    def simulateSubstitutedCircuit_VEC(nodeOrder, circuit, assignment, truthTable, responseFunctions, circuitSim, results,
        ...
        while (not stop):
            ...
            currentCircuitVals_VEC = {key: {node: np.zeros(N) for node in nodeOrder} for key in
                                  truthTable["inputs"]}
    ...
    def simulateCircuit_VEC(nodeOrder, circuit, assignment, truthTable, responseFunctions, circuitSim, results,
        ...
        for outputIdent in truthTable["outputs"]:
            ...
            circuitVals = getCircuitVal_VEC(nodeOrder, circuit, assignment, truthTable, bioInputs[inputIdent],
                                        dict(circuitSim), responseFunctions, N, substitution=substitutions[inputIdent],
            ...
            for outputID in outputIDs:
                results[outputID][truthTable["outputs"][outputIdent][outputID]][outputIdent] = circuitVals[
                    outputID]
    ...
    def getCircuitVal_VEC(nodeOrder, circuit, assignment, truthTable, circuitInputs_VEC, circuitVals_VEC, responseFunctions_VEC,
                      N, substitution={}, substitutionValues={}):
        inputIDs = truthTable["input_IDs"].keys()
        outputIDs = truthTable["output_IDs"].keys()
        ...
        for node in nodeOrder:
            ...
            if (node in inputIDs):
                ...
                value_VEC = np.empty(N)
                value_VEC.fill(circuitInputs_VEC[node])
                #print("1. " + str(circuitInputs_VEC[node]))
                ...
                value_VEC = circuitVals_VEC[node]
                #print("2. " + str(value_VEC))
            else:
            ...
            for child in circuit[node]:
                valueToPropagate_VEC = value_VEC if (not (nodeInSubstitution and child in substitution[node])) \
                    else valueToSubstitute  # The actual value propagated depends on whether the target gate requires a substitued gate or not.

                circuitVals_VEC[child] += valueToPropagate_VEC # VEC: numpy arrays are directly addable
                # print(node, circuitVals[node])
        # print("OUTPUT", circuitVals["NOT_1"])
        # print("")
        return circuitVals_VEC
...
while (inText != "exit"):
    ...
    if (cmd == "start"):
        ...
        if (simContext["assignment"] != "NULL"):
            ...
            startSimulation_VEC(assignment=simContext["assignment"], simData=simData, simSpec=simSpec)  # Perform simulation
