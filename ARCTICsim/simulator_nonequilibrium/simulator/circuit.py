...
class GeneticLogicCircuit:
    def __init__(self, structure: CircuitStructure):
        ...
        self.gene_circuit = GeneCircuit(self.structure, assignment=self.assignment)
    ...
    def set_assignment(self, assignment: CircuitAssignment):
        ...
        self.gene_circuit.set_assignment(assignment)
        pass
    ...
    def __call__(self, input_vals_dict, sim_settings):
        ...
        gene_circuit_inputs = None
        cell_state = self.gene_circuit(input_vals_dict=gene_circuit_inputs)
...
class GeneCircuit:
    def __init__(self, structure: CircuitStructure, assignment: CircuitAssignment = None):
        self._structure = structure
        self._assignment = assignment
        self.genes = {}
        self.gene_propagation_graph = {}
        self.genes_by_associated_promoter = {}
        self.genes_by_associated_cds = {}
        self.gene_evaluation_order = None
    ...
    @assignment.setter
    def assignment(self, assignment: CircuitAssignment):
    ...
    def create_gene_circuit(self):
        if self.structure is None:
            return
        # inputs = self.structure.inputs
        # gate_ids = self.structure.gates
        # num_inputs = len(inputs)
        # num_gates = len(gate_ids)
        #
        # gate_type_counts = {}
        # for gate_id in gate_ids:
        #     node_info = self.structure["node_infos"][gate_id]
        #     cur_type = node_info.type
        #     if cur_type not in gate_type_counts:
        #         gate_type_counts[cur_type] = 0
        #     gate_type_counts[cur_type] += 1

        # To derive number of gene expression units Count Gate Inputs which is equal to the edge count
        # NOT -> 1
        # NOR2 -> 2
        # OUTPUT_OR2 -> 2
        # OUTPUT_BUFFER -> 1

        # num_gene_expression_units = len(self.structure.edges)

        # num_TFs = len(self.structure.gates)
        ...
        for iD, (src_id, tar_id) in enumerate(self.structure.edges):
            ...
            # tar_id defines cds
            gene = Gene(id=f"Gene {iD}", promoter_associated_device=src_id, cds_associated_device=tar_id)
        ...
        self.gene_evaluation_order = gene_evaluation_order

    def _insert_assignment(self):
        if self.structure is None or self.assignment is None:
            return
        structure = self.structure
        assignment = self.assignment
        ...
        # for gene_id in genes:
        #     device_id = genes[gene_id].promoter_associated_device
        #     device_info = structure.node_infos[device_id]
        #     device = assignment(device_info)
        #
        #     # ToDo Extend Assignment Class and add required information.
        #     if device_info == "INPUT":
        #         genes[gene_id].promoter = device.promoter
        #     else:
        #
        #         # We are assining a gate or an output
        #         genes[gene_id].utr = device.utr
        #         genes[gene_id].cds = device.cds
        #         genes[gene_id].terminator = device.terminator
        #
        #         gene_propagation_graph[gene_id] = []
        #
        #         if not device_info.type.startswith("OUTPUT_"):
        #             for target_id in connection_graph[device_id]:
        #                 target_gene = genes_by_associated_device[target_id]
        #                 target_gene.promoter = device.promoter
        #                 gene_propagation_graph[gene_id].append(target_gene)
    ...
    def __call__(self, input_vals_dict: dict, sim_settings: dict, *args, **kwargs):
        ...
        # All quantities relevant for the cell are stored in this dict
        # Proteins
        # RNA
        # Small Molecules
        cell_state = {}
...
class Gene:
    ...
    def __call__(self, cell_state: dict, sim_settings: dict, *args, **kwargs):
        ...
        approximate = sim_settings["approximate"]
        ...
        if approximate:
            ...
            #
            interpolation_output = self.perform_interpolation(input_val_dict)
        ...
        energy_promoter = model_outputs["energy_dissipation_rate"]
        energy_rna = model_outputs["rna_mean_energy_dissipation_rate"]
        energy_protein = model_outputs["protein_mean_energy_dissipation_rate"]
        ...
        # raise Exception("Complete Implementation")
        return gene_state
    ...
    def init_interpolation(self) -> dict:
        # raise Exception("Not ")
        return {}
        input_vals = np.logspace()  # Define adequate spacing
        ...
        for func_id in Gene.func_ids:
            ...
            interpolators[func_id] = RegularGridInterpolator(X, Y)
...
if __name__ == '__main__':
    ...
    # assignment = CircuitAssignment(json_assignment, gate_lib=gatelib)
