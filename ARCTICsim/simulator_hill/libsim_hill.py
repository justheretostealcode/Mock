...
class nor_circuit:
    def __init__(self, structure, assignment, library, solver=None):
        ...
        self.lib = library
        self.gates = None
        self.wire_capacity = 1  # change that to something adaptive
    ...
    def solve(self, tol, max_iter):
        if self.solver is not None:
            ...
            # Place the output at the promoter activity position, i.e. 0
            return (values[self.node_idx[list(self.structure.outputs)[0]], 0], err, iter)
        return None
...
class _nor_gate(_base_gate):
    ...
    def __init__(self, node, dev, parameters):
        ...
        self.n = parameters['n']
        self.K = parameters['K']
        self.ymax = parameters['y_max']
        self.ymin = parameters['y_min']
    ...
    def out(self, input):
        ...
        # output contains two values
        # output[0] = outwired activity
        self.output[0] = self.dy/(1 + np.power(input[0]/self.K, self.n)) + self.ymin
        return self.output
    def __str__(self):
    ...
    def _debug_step(self, vs, run, err):
        ...
        for k, v in self.circuit.node_idx.items():
            if self.circuit.gates[self.circuit.node_idx[k]].type == 0:
                print('{k:<30}'.format(k = hl(k)) + ' (nor)' + ': ' + hl(str(vs[v])))
