...
class nor_circuit:
        ...
        for p, d in self.lib.parts['promoters'].items():
            ...
            for tf in self.tf_idx.keys():
                ...
                for pix in self.tf_idx[tf]:
                    ...
                    self.factors[0, self.p_idx[p], pix, :N] = float(d['f']['rnap']) * np.array(
                        list(map(float, d['f']['tf_rnap'][tf])))
        ...
        for k, v in self.assignment.map_gtod.items():
            ...
            if v[0] == '_':  # dummy gate
                ...
                self.gates[self.node_idx[k]] = _nor_gate(k, v, self.factors[:, self.dev_idx[v][0], :, :],
                                                         self.affinities[self.dev_idx[v][0]], self.lib.env['reservoir'])
            ...
            for attr, content in self.assignment.attributes[v].items():
                ...
                if isinstance(content, dict):
                    ...
                    for sattr, vals in content.items():
                        lut[_var_map[sattr], :, self.node_idx[target] + 1] = np.array(
                            [_var_pre[sattr](vals[0], self), _var_pre[sattr](vals[1], self)])
                        if DEBUG_LEVEL > 1:
                            print(hl(sattr) + '(s) ', end='')
                else:
                    lut[_var_map[attr], :, 0] = np.array(
                        [_var_pre[attr](content[0], self), _var_pre[attr](content[1], self)])
                    if DEBUG_LEVEL > 1:
                        print(hl(attr) + '(i) ', end='')
    ...
    def propagate_bound(self, a, mode_lut, force_map):
        ...
        for n in range(len(self.gates)):
                ...
                if m != n and target_gate.type == 0:
                    ...
                    if source_gate.type != -1:
                        ...
                        if source_gate.type == 0:
                            ...
                            p_a[self.g_p[m]] += source_gate.lut(
                                *mode_lut[_xtalk_assignedandnor, target_gate.env, source_gate.env])
                        else:
                            # -- crosstalk, assigned, source OR: assign the lut value
                            p_a[self.g_p[m]] += source_gate.lut(
                                *mode_lut[_xtalk_dummyoror_or, target_gate.env, source_gate.env])
                    # -- crosstalk: is the source gate assigned and a NOR or is it dummy/OR? -> dummy/OR
                        ...
                        pa_ix = int(
                            source_gate.lut(*mode_lut[_xtalk_dummyoror_tf, target_gate.env, source_gate.env], target=n))
                        ...
                        if source_gate.type == -1:
                            ...
                            p_a[pa_ix] += source_gate.lut(
                                *mode_lut[_xtalk_dummyoror_dummy, target_gate.env, source_gate.env])
                    if DEBUG_LEVEL > 2:
                        print(str('x') + str(') '), end='')
                else:
                    if DEBUG_LEVEL > 2:
            ...
            if DEBUG_LEVEL > 2:
                print('')
                print('p_a vector for ' + hl(str(target_gate.node)) + ' (index ' + hl(str(n)) + ', p_a index ' + hl(
                    str(self.g_p[n])) + ')' + ' of type ' + hl(str(target_gate.type)) + ':')
                print(p_a)
    ...
    def propagate(self, a):
        ...
        for n in range(len(self.gates)):
            ...
            gate = self.gates[n]
            if DEBUG_LEVEL > 2:
                print('p_a vector for ' + hl(str(gate.node)) + ' (index ' + hl(str(n)) + ', p_a index ' + hl(
                    str(self.g_p[n])) + ')' + ' of type ' + hl(str(gate.type)) + ':')
                print(p_a)
    ...
    def set_dummy_mode(self, ttix=0):
        ...
        if self.structure.gate_truthtables is None:
            if DEBUG_LEVEL > 0:
                print(
                    'Warning: ' + hl('no gate truthtables') + ' found! Bounding mode will most likely produce errors!')
            return
        for n in range(len(self.gates)):
...
class _nor_gate(_base_gate):
    ...
    def out(self, wa):
        ...
        if DEBUG_LEVEL > 2:
            print(str(self.bep + np.sum(iwa * self.bepj)) + '/' + str(1 + np.sum(iwa * self.bef)) + ' = ' + hl(
                str((self.bep + np.sum(iwa * self.bepj)) / (1 + np.sum(iwa * self.bef)))))
...
class _implicit_or_gate(_base_gate):
    ...
    def lut(self, var, val, target=-1, default=None):
        try:
            ...
            raise ValueError(
                'LUT assertion error occured in gate \'' + str(self.node) + '\' of type implicit OR') from e
...
class nor_circuit_solver_powell:
    ...
    def solve(self, tol=10 ** (-3), max_iter=100):
        ...
        if self.bound:
            function = (
                lambda a, _bc=_bounding_configs[self.mode], _fc=_force_configs[self.mode]: self.circuit.propagate_bound(
                    a, _bc, _fc))
        else:
            function = self.circuit.propagate
        vs = self.values
        ...
        result = so.root(obj, np.copy(vs[mutix]), tol=tol, method='lm',
                         options={'col_deriv': True, 'factor': 1.})  # , callback=(lambda x, f, r=[run]: callback))
...
#############################################################################
#   Pasted communication_wrapper writter by Nicolai Engelmann from          #
#   libsim.py to libsim_coop.py                                             #
#                                                                           #
#############################################################################
class communication_wrapper:
    def __init__(self, input_file, output_file, prefix=None):
